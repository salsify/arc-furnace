...
describe ArcFurnace::SuffixedFixedColumnCSVSink do
  let(:target_filename) { Dir::Tmpname.create('output.csv') {} }
  let(:expected_output_path) { "#{ArcFurnace.test_root}/resources/suffixed_fixed_column_expected.csv" }
  let(:sink) { ArcFurnace::SuffixedFixedColumnCSVSink.new(filename: target_filename, fields: { "id" => 1, "Field 1" => 2, "Field 2" => 1 }) }
